...
PQueue_p PQueue_constructor() {
    PQueue_p queue = (PQueue_p) malloc(sizeof(PQueue_p));
    queue->head = NULL;
    queue->rear = NULL;
    queue->size = 0;
    queue->counter = 0;
 ...
 */
void PQueue_destructor(PQueue_p queue) {

    // Free allocated memory for each Node in Queue
    while (queue->head != NULL)
        free(PQueue_dequeue(queue));
 ...
 */
int PQueue_get_size(PQueue_p queue) {
    return queue->size;
}
 ...
 */
int PQueue_get_counter(PQueue_p queue) {
 ...
 */
PQueue_p PQueue_enqueue(PQueue_p queue, PCB_p pcb) {
 ...
 */
PCB_p PQueue_dequeue(PQueue_p queue) {
 ...
 */
PCB_p PQueue_peek(PQueue_p queue) {
 ...
 */
void PQueue_print(PQueue_p queue, int printLastNode) {
    if (queue->size == 0) {
 ...
 */
char *PQueue_toString(PQueue_p queue, int printLastNode) {
